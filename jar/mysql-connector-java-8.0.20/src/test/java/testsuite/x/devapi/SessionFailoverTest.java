...
public class SessionFailoverTest extends DevApiBaseTestCase {
    ...
    /**
     * Tests xdevapi.library.test.connect-timeout and connectTimeout functionality.
     * 
     * The real socket library.test.connect timeout can be revealed only when trying to library.test.connect to the unavailable remote host
    ...
    public void testConnectionTimeout() throws Exception {
        ...
        // TS1_1 Create a session to a Server using explicit "xdevapi.library.test.connect-timeout" overriding implicit "connectTimeout".
        ...
        // TS1_2 Create a session to a Server using explicit "xdevapi.library.test.connect-timeout" overriding explicit "connectTimeout".
        ...
        // TS1_3 Create a session to a Server using explicit "connectTimeout" overriding implicit "xdevapi.library.test.connect-timeout".
        ...
        // TS3_1 Create a session to a remote offline host not setting the "library.test.connect-timeout" parameter. The connection must timeout in ~10 seconds.
        // Default "library.test.connect-timeout" (10000 ms) overrides implicit "connectTimeout".
        testConnectionTimeout_assertFailureTimeout(buildConnectionString(fakeHost), 10000, 11000);

        // TS4_1 Create a session to a remote offline host setting "library.test.connect-timeout" to zero (0). The connection must not timeout until cancelled.
        ...
        // The Server with the higher priority must be offline. The connection must succeed after library.test.connect-timeout milliseconds.
        ...
        // Both Servers must be offline. The connection must time out after library.test.connect-timeout * 2 milliseconds.
        ...
        // TS8_1 Create a session to a Server using valid credentials passing the "library.test.connect-timeout" and set it to a valid value.
        ...
        // TS11_1 Set connection property xdevapi.library.test.connect-timeout=null, try to create Session, check that WrongArgumentException is thrown
        // with message "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value 'null' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
                "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value 'null' can not be converted to an integer.",
                () -> this.fact.getSession(buildConnectionString(fakeHost, this.testsHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "null", true)));

        // TS11_2 Set connection property xdevapi.library.test.connect-timeout=-1, try to create Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values in the range of 0 - 2147483647, the value '-1' exceeds this range."
        assertThrows(WrongArgumentException.class,
                "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values in the range of 0 - 2147483647, the value '-1' exceeds this range.",
                () -> this.fact.getSession(buildConnectionString(fakeHost, this.testsHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "-1", true)));

        // TS11_3 Set connection property xdevapi.library.test.connect-timeout=abc, try to create Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value 'abc' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
                "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value 'abc' can not be converted to an integer.",
                () -> this.fact.getSession(buildConnectionString(fakeHost, this.testsHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "abc", true)));

        // TS11_4 Set connection property xdevapi.library.test.connect-timeout=, try to create Session, check that WrongArgumentException is thrown with
        // message "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value '' can not be converted to an integer."
        assertThrows(WrongArgumentException.class,
                "The connection property 'xdevapi.library.test.connect-timeout' only accepts integer values. The value '' can not be converted to an integer.",
                () -> this.fact.getSession(buildConnectionString(fakeHost, this.testsHost) + "?" + makeParam(PropertyKey.xdevapiConnectTimeout, "", true)));

        // TS11_5 Set connection property xdevapi.library.test.connect-timeout=12.8. Please note that c/J truncates decimals w/o exception for integer parameters thus
        // the error message is not thrown against the property value. Try to library.test.connect with this connection string and ensure that CJCommunicationsException
        ...
        // TS12_1 Create a session to a Server giving a valid value for the "library.test.connect-timeout", and use invalid credentials.
